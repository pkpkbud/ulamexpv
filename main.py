...
def _dwupodporowa():
    ...
    ilosc_kons = ceil(ilosc_pv / 12) if typ_pv in [1, 11] else ceil(ilosc_pv / 8)
    result = (
        ...
        f'<h2><img src="/static/Dwupodporowa_{1 if typ_pv in [1, 11] else 2}{uklad_pv}.png"> x {ilosc_kons}</h2>'
    ...
    if typ_pv in [1, 11] and uklad_pv == 1:
        result += (
    ...
    if typ_pv in [1, 11] and uklad_pv == 2:
        result = "<h2>BRAK</h2>"
